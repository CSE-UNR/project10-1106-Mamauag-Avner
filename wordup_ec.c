...
#define READ_FILENAME "mystery_ec.txt"
#define MAX_WORDS 50
#define WORD_LEN 6
...
int main(){
	FILE* wordFilePtr;
	char wordList[MAX_WORDS][WORD_LEN], word[WORD_LEN];
	int wordCount = 0, wordIndex;
	...
	while(fscanf(wordFilePtr, "%s", wordList[wordCount]) == 1){
		wordCount++;
	}
	fclose(wordFilePtr);
	...
	int j;
	for(j = 0; wordList[wordIndex][j] != '\0'; j++){
